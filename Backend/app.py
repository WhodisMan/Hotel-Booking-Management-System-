...
def predict():
    ...
    try:
        ...
        return jsonify({'error': str(e)})
    print(res[0][0].strftime('%Y-%m-%d'))
    dictToSend={"info":[res[0][0].strftime('%Y-%m-%d'),res[0][1].strftime('%Y-%m-%d'),res[0][2].strftime('%Y-%m-%d'),res[0][3],res[0][4]]}
    rlt = requests.post('http://localhost:5000/predict', json=dictToSend,headers={'content-type': 'application/json'})
    print(rlt.json())

    return jsonify({'result':"Won't Cancel" if rlt.json()['data']=='1' else "Will Cancel"})
